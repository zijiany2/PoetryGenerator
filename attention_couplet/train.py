...
from find_rhyme_feature import SEP_TOKEN, PAD_TOKEN, VOCAB_SIZE, MODEL_DIR, EMBEDDING_DIM, gen_batch_train_data, appended_embedding
from model import Seq2SeqModel

# Data loading parameters
tf.app.flags.DEFINE_boolean('cangtou_data', False, 'Use cangtou training data')
tf.app.flags.DEFINE_boolean('rev_data', False, 'Use reversed training data')
tf.app.flags.DEFINE_boolean('align_data', False, 'Use aligned training data')
tf.app.flags.DEFINE_boolean('prev_data', False, 'Use training data with previous sentences')
tf.app.flags.DEFINE_boolean('align_word2vec', False, 'Use aligned word2vec model')
...
tf.app.flags.DEFINE_integer('hidden_units', EMBEDDING_DIM, 'Number of hidden units in each layer')
tf.app.flags.DEFINE_integer('depth', 3, 'Number of layers in each encoder and decoder')
tf.app.flags.DEFINE_integer('embedding_size', EMBEDDING_DIM, 'Embedding dimensions of encoder and decoder inputs')
tf.app.flags.DEFINE_integer('num_encoder_symbols', VOCAB_SIZE, 'Source vocabulary size')
tf.app.flags.DEFINE_integer('num_decoder_symbols', VOCAB_SIZE, 'Target vocabulary size')
...
tf.app.flags.DEFINE_integer('max_seq_length', 7, 'Maximum sequence length')
...
tf.app.flags.DEFINE_string('model_name', 'couplet.ckpt', 'File name used for model checkpoints')
...
def train():
    ...
    with tf.Session(config=config_proto) as sess:
        ...
        # Load appended embedding
        embedding = appended_embedding
        ...
        for epoch_idx in xrange(FLAGS.max_epochs):
            ...
            for source, source_len, target, target_len in gen_batch_train_data(FLAGS.batch_size):
                #print source.shape, source_len.shape, target.shape, target_len.shape
