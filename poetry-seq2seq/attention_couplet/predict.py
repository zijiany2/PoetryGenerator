...
from find_rhyme_feature import transfer_batch_predict_data, ints_to_sentence
...
class Seq2SeqPredictor:
    ...
    def predict(self, firstline): ##MODIFIED!!!!
        source, source_len = transfer_batch_predict_data(firstline)        
    ...
    def predict_with_couplet(self, keywords):
        ...
        for keyword in keywords[::2]:
            ...
            self.soc.send(predicted_sentence.encode("utf8")) # we must encode the string to bytes  
            result_bytes = self.soc.recv(4096) # the number means how the response can be in bytes 
            self.soc.close() 

            pair_sentence = result_bytes.decode("utf8") # the return will be in bytes, so decode
