...
def get_corpus(covert_to_ints=True, seq_len=50):
    padded_pairs = [(pair[0] + '_' * (seq_len - len(pair[0])),
        pair[1] + '_' * (seq_len - len(pair[1]))) for pair in all_pairs if len(pair[0]) <= seq_len]
...
couplet_path = abspath(join(dirname(__file__), 'duizhang_space.txt'))
all_pairs = load_corpus_b(couplet_path)
# all_pairs = cb + ca + cc
# print(len(all_pairs))
...
all_chars = ['_'] + sorted(list(all_chars))
